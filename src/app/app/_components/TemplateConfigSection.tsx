...
import { Section, SectionType } from '@/types/template.types';
...
export default function TemplateConfigSection<T extends FieldValues>({
    ...
    return (
        ...
        >
            ...
            >
                {
                    section.type === 'list' ? <SectionList section={section} /> : <SectionObject<T> section={section} />
                }
            </div>
        </div>
    );
...
function SectionObject<T extends FieldValues>({ section }: { section: Section }) {
    ...
    )} style={{ '--tpl-section-cols': section.gridCols || '1' } as React.CSSProperties}>{section.fields.map((field: Field) => {
        return (
            <div
                key={`template-wrapper-field-${field.name}`}
                className={cn('space-y-1')} style={
                    {
...
function SectionList({ section }: { section: Section }) {
    return (<div >
        <ArrayInputField fieldName={section.name} field={{ name: section.name, fields: section.fields, gridCols: section.gridCols }} />
    </div>);
}
